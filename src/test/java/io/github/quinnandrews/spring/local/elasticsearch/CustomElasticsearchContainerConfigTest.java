...
public class CustomElasticsearchContainerConfigTest {
    ...
    void indexReadable_getPedalById() {
        ...
        final var optionalPedal = guitarPedalDocumentRepository.findById(2L);
        // then the pedal associated with that id is returned
        assertTrue(optionalPedal.isPresent());
        assertEquals(2L, optionalPedal.get().getId());
        assertEquals("Strymon Deco: Tape Saturation and Double Tracker", optionalPedal.get().getName());
    }
}
